...
with Engine(custom_parser=parser) as engine:
    ...
    # 动态调整学习率的调度器，使用预热和多项式衰减策略来帮助训练过程
    lr_policy = WarmUpPolyLR(base_lr, config.lr_power, total_iteration, config.niters_per_epoch * config.warm_up_epoch)
    
    # device_ids 参数指定了当前进程使用的 GPU 设备 ID 通常为 engine.local_rank，表示每个进程使用的本地 GPU
    # output_device 参数也设置为 engine.local_rank，这指定了输出将发送到哪个设备
    if engine.distributed:
            ...
            if dist.get_rank() == 0:
                ...
                if (epoch >= config.checkpoint_start_epoch) and (epoch - config.checkpoint_start_epoch) % config.checkpoint_step == 0:
                    ...
                    with torch.no_grad():
                        ...
                        _, mean_IoU = segmentor.run(config.checkpoint_dir, str(epoch), config.val_log_file, config.link_val_log_file)
