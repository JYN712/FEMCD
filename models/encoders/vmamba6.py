...
from ..decoders.DDLNet import MultiSpectralAttentionLayer
...
if True:
    ...
    class CrossScan_decoder(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x: torch.Tensor):
            B, C, H, W = x.shape
            ctx.shape = (B, C, H, W)
            ...
            return xs # torch.Size([1, 8, 192, 4096])
    ...
    class CrossMerge_decoder(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor):
            B, K, D, H, W = ys.shape
            ctx.shape = (H, W)
    ...
    class CrossScan_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x_rgb: torch.Tensor, x_e: torch.Tensor): # x_rgb:[2, 192, 64, 64], x_e:[2, 192, 64, 64]
            # B, C, H, W -> B, 2, C, 2 * H * W
            B, C, H, W = x_rgb.shape # (2, 192, 64, 64)
            ctx.shape = (B, C, H, W) # 一个上下文对象，用于在前向传播和反向传播之间保存和传递信息。
            xs_fuse = x_rgb.new_empty((B, 2, C, 2 * H * W)) # torch.Size([2, 2, 192, 8192])
            xs_fuse[:, 0] = torch.concat([x_rgb.flatten(2, 3), x_e.flatten(2, 3)], dim=2)
            xs_fuse[:, 1] = torch.flip(xs_fuse[:, 0], dims=[-1]) # 反转张量
            # xs_fuse[:, 2, :, 0:H*W] =  torch.flatten(x_diff, 2)
            # xs_fuse[:, 2, :, H*W:2*H*W] =  torch.flip(xs_fuse[:, 2, :, 0:H*W], dims=[-1])
            return xs_fuse # torch.Size([1, 3, 192, 8192])
        ...
        def backward(ctx, ys: torch.Tensor):
            ...
            B, C, H, W = ctx.shape
            L = 2 * H * W
            zs = ys[:, 0] + ys[:, 1].flip(dims=[-1]) # B, d, 2 * H * W
            # diffs = ys[:, 2, :, 0:H*W] + torch.flip(ys[:, 2, :, H*W:H*W*2], dims=[-1])
            # get B, d, H*W
            return zs[:, :, 0:H*W].view(B, -1, H, W), zs[:, :, H*W:2*H*W].view(B, -1, H, W)   # diffs.view(B, -1, H, W)

    class CrossScan_multimodal_dif(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x: torch.Tensor):
            B, C, H, W = x.shape # torch.Size([1, 192, 64, 64])
            ctx.shape = (B, C, H, W)
    ...
    # ys:torch.Size([2, 3, 192, 8192]), y_rgb, y_e:[2, 192, 4096]
    class CrossMerge_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor): # torch.Size([1, 2, 192, 8192])
            B, K, D, L = ys.shape
        ...
        @staticmethod
        def backward(ctx, x1: torch.Tensor, x2: torch.Tensor): # [1, 1536, 64], [1, 1536, 64], [1, 1536, 64]
            B, C, L = x1.shape
    ...
    class CrossMerge_multimodal_dif(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor): # torch.Size([1, 2, 192, 4096])
            B, K, D, L = ys.shape
            ys = ys[:, 0] + ys[:, 1].flip(dims=[-1]) # B, d, 2 * H * W, broadcast
            return ys[:, :, 0:L//2], ys[:, :, L//2:L]
    ...
    def cross_selective_scan(
        ...
        B, D, H, W = x.shape # torch.Size([1, 192, 64, 64])
        D, N = A_logs.shape
        K, D, R = dt_projs_weight.shape
        L = H * W
        # x:torch.Size([3, 192, 64, 64])
        if nrows < 1:
            if D % 4 == 0:
                nrows = 4
            elif D % 3 == 0:
        ...
        y = CrossMerge.apply(ys) # torch.Size([3, 192, 4096])
        ...
        return y # torch.Size([3, 64, 64, 192])

    def cross_selective_scan_dec(
        x: torch.Tensor=None, 
        x_proj_weight: torch.Tensor=None,
        x_proj_bias: torch.Tensor=None,
        dt_projs_weight: torch.Tensor=None,
        ...
        y = CrossMerge_decoder.apply(ys) # torch.Size([3, 192, 4096])
    ...
    def cross_selective_scan_multimodal_k2(
        ...
        x_e: torch.Tensor=None,
        x_proj_weight: torch.Tensor=None,
        x_proj_weight_dif: torch.Tensor=None,
        x_proj_bias: torch.Tensor=None,
        dt_projs_weight: torch.Tensor=None,
        dt_projs_weight_dif: torch.Tensor=None,
        dt_projs_bias: torch.Tensor=None,
        dt_projs_bias_dif: torch.Tensor=None,
        A_logs: torch.Tensor=None,
        A_logs_diff: torch.Tensor=None,
        Ds: torch.Tensor=None,
        Ds_dif: torch.Tensor=None,
        out_norm1: torch.nn.Module=None,
        out_norm2: torch.nn.Module=None,
        out_norm3: torch.nn.Module=None,
        softmax_version=False,
        nrows = -1,
        delta_softplus = True,
    ):
        ...
        x_fuse_diff = CrossScan_multimodal_dif.apply(x_diff)

        x_dbl_diff = torch.einsum("b k d l, k c d -> b k c l", x_fuse_diff, x_proj_weight_dif)

        dts_dif, Bs_dif, Cs_dif = torch.split(x_dbl_diff, [R, N, N], dim=2)
        dts_dif = torch.einsum("b k r l, k d r -> b k d l", dts_dif, dt_projs_weight_dif)
        x_fuse_diff = x_fuse_diff.view(B, -1, L_diff).to(torch.float)

        dts_dif = dts_dif.contiguous().view(B, -1, L_diff).to(torch.float)
        As_dif = -torch.exp(A_logs_diff.to(torch.float)) # (k * c, d_state)
        Bs_dif = Bs_dif.contiguous().to(torch.float)
        Cs_dif = Cs_dif.contiguous().to(torch.float)
        Ds_dif = Ds_dif.to(torch.float) # (K * c)
        delta_bias_dif = dt_projs_bias_dif.view(-1).to(torch.float)
        ...
        diffs: torch.Tensor = selective_scan(
            x_fuse_diff, dts_dif, As_dif, Bs_dif, Cs_dif, Ds_dif, delta_bias_dif, delta_softplus, nrows,
        ).view(B, K, -1, H*W)

        ys[:, :, :, 0:L//2] = ys[:, :, :, 0:L//2] + diffs
        ys[:, :, :, L//2:L] = ys[:, :, :, L//2:L] + diffs
        # crossMerge
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs): # torch.Size([3, 64, 64, 96])
        xz = self.in_proj(x) # torch.Size([3, 64, 64, 384])
        if self.d_conv > 1:
            x, z = xz.chunk(2, dim=-1) # (b, h, w, d)
            x = x.permute(0, 3, 1, 2).contiguous()
            x = self.act(self.conv2d(x)) # (b, d, h, w)
            y = self.forward_core(x) # torch.Size([3, 192, 64, 64])
            if self.softmax_version:
                y = y * z
            else:
                y = y * F.silu(z)
        ...
        return out # torch.Size([3, 64, 64, 96])
...
class ConMB_SS2D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, N, inner)
        self.x_proj_weight_dif = nn.Parameter(torch.stack([t.weight for t in self.x_proj_dif], dim=0)) # (K, N, inner)
        del self.x_proj
        ...
        self.dt_projs_weight_dif = nn.Parameter(torch.stack([t.weight for t in self.dt_projs_dif], dim=0)) # (K, inner, rank)
        self.dt_projs_bias_dif = nn.Parameter(torch.stack([t.bias for t in self.dt_projs_dif], dim=0)) # (K, inner)
        ...
        # self.k3 = self.K // 2
        self.A_logs_diff = self.A_log_init(self.d_state, self.d_inner, copies=self.K, merge=True) # (K * D, N)
        self.Ds = self.D_init(self.d_inner, copies=self.K, merge=True) # (K * D)
        self.Ds_dif = self.D_init(self.d_inner, copies=self.K, merge=True) # (K * D)
    ...
    def forward_corev2_multimodal(self, x_rgb: torch.Tensor, x_e: torch.Tensor, nrows=-1):
        return cross_selective_scan_multimodal_k2(
            x_rgb, x_e, self.x_proj_weight, self.x_proj_weight_dif, None, self.dt_projs_weight, self.dt_projs_weight_dif, self.dt_projs_bias, self.dt_projs_bias_dif,
            self.A_logs, self.A_logs_diff, self.Ds, self.Ds_dif, getattr(self, "out_norm1", None), getattr(self, "out_norm2", None), 
            getattr(self, "out_norm3", None), self.softmax_version, 
            nrows=nrows,
        )

    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        # Linear
        x_rgb = self.in_proj(x_rgb) # b,h,w,c
        x_e = self.in_proj_modalx(x_e) # b,h,w,c
        ...
        if self.d_conv > 1:
            ...
            y_rgb = y_rgb * x_e_exitation # torch.Size([2, 64, 64, 192])   torch.Size([2, 1, 1, 192])
...
class CVSSDecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv2 = LightweightModel(in_channels=hidden_dim, out_channels=hidden_dim)

        # small
        c2wh = dict([(96,56), (192,28), (384,14) ,(768,7)])

        self.fca = MultiSpectralAttentionLayer(hidden_dim, c2wh[hidden_dim], c2wh[hidden_dim],  reduction=16, freq_sel_method = 'top16')


    def _forward(self, input: torch.Tensor):
        # DCT branch
        dct = self.fca(input.permute(0, 3, 1, 2))
        # MDVSS branch
...
class ConcatMambaFusionBlock(nn.Module):
    ...
    # b,c,h,w
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        x = self.drop_path(self.op(x_rgb, x_e)) # torch.Size([1, 64, 64, 96])
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN
        return x
...
class SS2D_DEC(nn.Module):
    ...
    def forward_corev2(self, x: torch.Tensor, nrows=-1):
        return cross_selective_scan_dec(
            x, self.x_proj_weight, None, self.dt_projs_weight, self.dt_projs_bias,
            self.A_logs, self.Ds, getattr(self, "out_norm", None), self.softmax_version, 
            nrows=nrows,
        )
