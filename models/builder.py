...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        elif cfg.backbone == 'sigma_small':  # there
            logger.info('Using backbone: V-MAMBA')
            self.channels = [96, 192, 384, 768]
    ...
    def encode_decode(self, rgb, modal_x):
        ...
        if not self.deep_supervision:
            orisize = rgb.shape
            # [4, 96, 64, 64],[4, 192, 32, 32],[4, 384, 16, 16],[4, 768, 8, 8]
            x = self.backbone(rgb, modal_x) # out:四个不同尺度的特征

            # print(self.decode_head)
            out = self.decode_head.forward(x) # torch.Size([1, 2, 256, 256])
